@php
    ...
    if (!empty($card->parkAccommodation)) {
        $displayTitle = Str::slug($card->base->title);
        $accommodationUrl = route('park-accommodation.book', ['title' => $displayTitle, 'id' => $card->id]);
    }
@endphp
...
<div class="accommodation-item" data-accommodation_id="{{ $card->id }}"
    ...
    <div class="right-container">
        ...
        <div class="ctas">
            ...
            @endif
            @if (!empty($card->parkAccommodation))
                <a class="btn secondary-btn booking-button" href="{{ $accommodationUrl }}">
                    Book Now
                </a>
            @endif
        </div>
